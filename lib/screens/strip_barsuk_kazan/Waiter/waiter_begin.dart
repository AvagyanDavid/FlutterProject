...
class _Waiter_beginState extends State<Waiter_begin> {
  ...
  void getStatusReport() async {
    ...
    if (results != null) {
      ...
      commentDirectorTableArrangment = response['CommentDirector_TableArrangment'];
      wipeTheTables = response['WipeTheTables'] != 0 ? true : false;
      commentWipeTheTables.text = response['Comment_WipeTheTables'];
      commentDirectorWipeTheTables = response['CommentDirector_WipeTheTables'];
      ...
      commentDirectorInspectionOfTheHall = response['CommentDirector_InspectionOfTheHall'];
      arrangeOttomans = response['ArrangeOttomans'] != 0 ? true : false;
      commentArrangeOttomans.text = response['Comment_ArrangeOttomans'];
      commentDirectorArrangeOttomans = response['CommentDirector_ArrangeOttomans'];
      ...
      commentDirectorPutEverithingOnTheTables = response['CommentDirector_PutEverithingOnTheTables'];
      wipeMenu = response['WipeMenu'] != 0 ? true : false;
      commentWipeMenu.text = response['Comment_WipeMenu'];
      commentDirectorWipeMenu = response['CommentDirector_WipeMenu'];
      cleanWineCabinet = response['CleanWineCabinet'] != 0 ? true : false;
      commentCleanWineCabinet.text = response['Comment_CleanWineCabinet'];
      commentDirectorCleanWineCabinet = response['CommentDirector_CleanWineCabinet'];
      ...
      commentDirectorFillTheNapkinHolder = response['CommentDirector_FillTheNapkinHolder'];
      garbageEmpty = response['GarbageEmpty'] != 0 ? true : false;
      commentGarbageEmpty.text = response['Comment_GarbageEmpty'];
      commentDirectorGarbageEmpty = response['CommentDirector_GarbageEmpty'];
      ...
      commentDirectorPassDishesKitchen = response['CommentDirector_PassDishesKitchen'];
      ...
      commentDirectorRequestStartAndStopList = response['CommentDirector_RequestStartAndStopList'];
      cleanHumidor = response['CleanHumidor'] != 0 ? true : false;
      commentCleanHumidor.text = response['Comment_CleanHumidor'];
      commentDirectorCleanHumidor = response['CommentDirector_CleanHumidor'];
                  ...
                  children: [
                    ...
                    BlocProvider<CheckboxBloc>(
                      ...
                      child: BlocBuilder<CheckboxBloc, CheckboxState>(
                        builder: (context, state) {
                          if (enabled == false) {
                            return ShowCheck(text: 'Расставить столы на равном расстоянии от диванов', value: tableArrangement);
                          } else {
                            return NewCheck(
                    ...
                    BlocProvider<PhotoBloc>(
                      ...
                      child: BlocBuilder<PhotoBloc, PhotoState>(
                        builder: (context, state) {
                          if (enabled == false) {
                            return ShowPhoto(image: tableArrangementPath);
                          } else {
                            return NewPhoto(
                              photoBloc: photoBloc,
                    ...
                    BlocProvider<CheckboxBloc>(
                      ...
                      child: BlocBuilder<CheckboxBloc, CheckboxState>(
                        builder: (context, state) {
                          if (enabled == false) {
                            return ShowCheck(text: 'Протереть стоы во всех местах', value: wipeTheTables);
                          }
                          return NewCheck(
                    ...
                    BlocProvider<PhotoBloc>(
                      ...
                      child: BlocBuilder<PhotoBloc, PhotoState>(
                        builder: (context, state) {
                          if (enabled == false) {
                            return ShowPhoto(image: wipeTheTablesPath);
                          } else {
                            return NewPhoto(
                              photoBloc: photoBloc,
                    ...
                    BlocProvider<CheckboxBloc>(
                      ...
                      child: BlocBuilder<CheckboxBloc, CheckboxState>(
                        builder: (context, state) {
                          if (enabled == false) {
                            return ShowCheck(text: 'Осмотреть зал на предмет мелкого мусора. При необходимости убрать', value: inspectionOfTheHall);
                          } else {
                            return NewCheck(
                    ...
                    BlocProvider<PhotoBloc>(
                      ...
                      child: BlocBuilder<PhotoBloc, PhotoState>(
                        builder: (context, state) {
                          if (enabled == false){
                            return ShowPhoto(image: inspectionOfTheHallPath);
                          } else {
                            return NewPhoto(
                              photoBloc: photoBloc,
                    ...
                    BlocProvider<CheckboxBloc>(
                      ...
                      child: BlocBuilder<CheckboxBloc, CheckboxState>(
                        builder: (context, state) {
                          if (enabled == false) {
                            return ShowCheck(text: 'Расставить ровна все пуфики', value: arrangeOttomans);
                          } else {
                            return NewCheck(
                    ...
                    BlocProvider<PhotoBloc>(
                      ...
                      child: BlocBuilder<PhotoBloc, PhotoState>(
                        builder: (context, state) {
                          if (enabled == false) {
                            return ShowPhoto(image: arrangeOttomansPath);
                          } else {
                            return NewPhoto(
                              photoBloc: photoBloc,
                      ...
                      child: BlocBuilder<CheckboxBloc, CheckboxState>(
                        builder: (context, state) {
                          if (enabled == false) {
                            return ShowCheck(
                                text:
                                    'Выставить все ровно на столах: салфетницы справа от кнопки, свечи слева от кнопки и крейзи меню слева от свечи',
                                value: putEverythingOnTheTables);
                          } else {
                            return NewCheck(
                              text: "Выставить всё ровно на столах: салфетницы справа от кнопки, свечи слева от кнопки и крейзи меню слева от свечи",
                    ...
                    BlocProvider<PhotoBloc>(
                      ...
                      child: BlocBuilder<PhotoBloc, PhotoState>(
                        builder: (context, state) {
                          if (enabled == false){
                            return ShowPhoto(image: putEverythingOnTheTablesPath);
                          } else {
                            return NewPhoto(
                              photoBloc: photoBloc,
                    ...
                    BlocProvider<CheckboxBloc>(
                      ...
                      child: BlocBuilder<CheckboxBloc, CheckboxState>(
                        builder: (context, state) {
                          if (enabled == false) {
                            return ShowCheck(text: 'Протереть все крейзи меню и меню бара', value: wipeMenu);
                          } else {
                            return NewCheck(
                    ...
                    BlocProvider<PhotoBloc>(
                      ...
                      child: BlocBuilder<PhotoBloc, PhotoState>(
                        builder: (context, state) {
                          if (enabled == false){
                            return ShowPhoto(image: wipeMenuPath);
                          } else {
                            return NewPhoto(
                              photoBloc: photoBloc,
                    ...
                    BlocProvider<CheckboxBloc>(
                      ...
                      child: BlocBuilder<CheckboxBloc, CheckboxState>(
                        builder: (context, state) {
                          if (enabled == false) {
                            return ShowCheck(text: 'Протереть винный шкаф', value: cleanWineCabinet);
                          } else {
                            return NewCheck(
                    ...
                    BlocProvider<CheckboxBloc>(
                      ...
                      child: BlocBuilder<CheckboxBloc, CheckboxState>(
                        builder: (context, state) {
                          if (enabled == false) {
                            return ShowCheck(text: 'Заполнить все салфетницы', value: fillTheNapkinHolder);
                          } else {
                            return NewCheck(
                    ...
                    BlocProvider<CheckboxBloc>(
                      ...
                      child: BlocBuilder<CheckboxBloc, CheckboxState>(
                        builder: (context, state) {
                          if (enabled == false) {
                            return ShowCheck(text: 'Ведерко для мусора должно быть пустым', value: garbageEmpty);
                          } else {
                            return NewCheck(
                    ...
                    BlocProvider<CheckboxBloc>(
                      ...
                      child: BlocBuilder<CheckboxBloc, CheckboxState>(
                        builder: (context, state) {
                          if (enabled == false) {
                            return ShowCheck(text: 'Передать посуду на кухне', value: passDishesKitchen);
                          }
                          return NewCheck(
                    ...
                    BlocProvider<CheckboxBloc>(
                      ...
                      child: BlocBuilder<CheckboxBloc, CheckboxState>(
                        builder: (context, state) {
                          if (enabled == false) {
                            return ShowCheck(text: 'Зарпосить стоп-лист и старт-лист по кухне и бару', value: requestStartAndStopList);
                          } else {
                            return NewCheck(
                    ...
                    BlocProvider<CheckboxBloc>(
                      ...
                      child: BlocBuilder<CheckboxBloc, CheckboxState>(
                        builder: (context, state) {
                          if (enabled == false) {
                            return ShowCheck(text: 'Протереть хьюмидор', value: cleanHumidor);
                          } else {
                            return NewCheck(
